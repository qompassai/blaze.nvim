...
function M.detect_runtime()
    local runtimes = {
        {name = 'docker', cmd = 'docker'}, {name = 'podman', cmd = 'podman'},
        {name = 'nerdctl', cmd = 'nerdctl'}
    }
    for _, runtime in ipairs(runtimes) do
        if vim.fn.executable(runtime.cmd) == 1 then return runtime.name end
    end
    return nil
end
function M.get_runtime_cmd()
    if M.config.runtime == 'auto' then
        ...
        if detected then
            ...
            vim.notify(
                'No container runtime found (docker, podman, or nerdctl)',
    ...
    if vim.fn.executable(M.config.runtime) == 1 then
        ...
        vim.notify('Configured runtime ' .. M.config.runtime .. ' not found',
                   vim.log.levels.ERROR)
        return nil
    end
end
...
function M.run_command(cmd, opts)
    ...
    local mount = M.config.mount_current and
                      ('-v %s:%s'):format(vim.fn.getcwd(), M.config.working_dir) or
                      ''
    ...
    local container_cmd = string.format('%s %s run --rm %s %s %s', runtime,
                                        rootless_flags, mount, M.config.image,
                                        cmd)
    if opts.async then
...
function M.setup(opts)
    ...
    local detected = M.detect_runtime()
    if detected then
        vim.notify('Using container runtime: ' ..
                       (M.config.runtime == 'auto' and detected or
                           M.config.runtime), vim.log.levels.INFO)
    else
        vim.notify(
