-- ~/.config/nvim/lua/blaze/container.lua
local M = {}
...
function M.run_command(cmd, opts)
  ...
  local mount = M.config.mount_current and ("-v %s:%s"):format(vim.fn.getcwd(), M.config.working_dir) or ""
...
function M.setup(opts)
  ...
  local detected = M.detect_runtime()
  if detected then
    vim.notify("Using container runtime: " .. (M.config.runtime == "auto" and detected or M.config.runtime),
               vim.log.levels.INFO)
  else
    vim.notify("No container runtime found. Please install docker, podman, or nerdctl.", vim.log.levels.WARN)
  end
