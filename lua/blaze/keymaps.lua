...
M.blaze_keymaps = function(opts)
    opts = opts or {}
    local pixi_prefix = opts.pixi and opts.pixi.keymaps and
                            opts.pixi.keymaps.prefix or '<leader>p'
  ...
  vim.fn.jobstart({ "mojo", "format", file }, {
    ...
    local function shell_cmd(cmd) return function() vim.cmd('!' .. cmd) end end
    ...
    local mojo_commands = {
        ...
        C = {
            shell_cmd('magic completion --shell bash'), '🧩 Shell Completion'
        },
    ...
    local pixi_commands = {
        ...
        I = {
            function() vim.cmd('PixiInstall') end, '⬇️ Install dependencies'
        },
        c = {
            function() vim.cmd('PixiCompletion') end, '🧩 Generate completion'
        },
        g = {function() vim.cmd('PixiGlobal') end, '🌍 Global management'},
        A = {
            function() vim.cmd('PixiAuth') end, '🔑 Login to package servers'
        },
    ...
    if wk_ok then
        wk.register({['<leader>m'] = {name = '+🔥 Mojo'}})

        for key, mapping in pairs(mojo_commands) do
            wk.register({[key] = mapping}, {prefix = '<leader>m'})
        end

        wk.register({[pixi_prefix] = {name = '+🧪 Pixi'}})

        for key, mapping in pairs(pixi_commands) do
            wk.register({[key] = mapping}, {prefix = pixi_prefix})
        end
    else
        for key, mapping in pairs(mojo_commands) do
            local mapping_opts = vim.tbl_extend('force', key_opts,
                                                {desc = mapping[2]})
